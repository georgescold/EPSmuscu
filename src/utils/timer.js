...
export function calculateTimerState(config, status) {
    ...
    if (status.state === 'running') {
        ...
        // idle - totalRemaining should be the full duration
        const phases = config.phases || []
        const repeats = config.repeats || 1
        const totalDuration = phases.reduce((acc, p) => acc + ((Number(p.duration) || 0) * 1000), 0) * repeats
        return {
            currentPhaseIndex: 0,
            currentPhase: phases[0] || null,
            remainingInPhase: (phases[0]?.duration || 0) * 1000,
    ...
    if (flatPhases.length === 0 || elapsed >= totalDuration) {
        isFinished = true
        return {
            currentPhaseIndex: Math.max(0, flatPhases.length - 1),
            currentPhase: flatPhases.length > 0 ? flatPhases[flatPhases.length - 1] : null,
            remainingInPhase: 0,
            totalRemaining: 0,
            isFinished: true,
