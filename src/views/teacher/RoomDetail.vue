  ...
  <div class="min-h-screen bg-gray-50 p-4 md:p-8">
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div v-if="activeTab === 'workshops'">
        <div v-if="isSavingOrder" class="text-xs text-gray-400 flex items-center justify-end mb-2">
           <Loader2 class="animate-spin w-3 h-3 mr-1"/> Sauvegarde de l'ordre...
        </div>
              ...
              <table class="w-full text-sm text-left">
                 ...
                 <tbody>
                    <tr class="border-b border-gray-100">
                       ...
                       <td v-if="notebookConfig.notebook_visible_series" class="px-4 py-3 text-gray-400">1</td>
                       <td v-if="notebookConfig.notebook_visible_level" class="px-4 py-3 text-gray-400">--</td>
                       <td v-if="notebookConfig.notebook_visible_charges" class="px-4 py-3 text-gray-400">--</td>
                       <td v-if="notebookConfig.notebook_visible_reps" class="px-4 py-3 text-gray-400">--</td>
...
// Timer setup is done in the main onMounted below
// onBeforeUnmount: clean up only this component's channels
onBeforeUnmount(() => {
   if (timerInterval) clearInterval(timerInterval)
   if (timerChannel) timerChannel.unsubscribe()
...
const subscribeToStudents = () => {
  studentChannel = supabase
    .channel(`public:students:${roomId}`)
...
const saveWorkshop = async () => {
  ...
  // Preserve existing order_index on update, only set for new workshops
  const existingWorkshop = editingWorkshopId.value
    ? workshops.value.find(w => w.id === editingWorkshopId.value)
    : null
  ...
  const payload = {
      ...
      order_index: existingWorkshop ? existingWorkshop.order_index : workshops.value.length,
...
onMounted(() => {
  ...
  // Timer realtime subscription
  timerChannel = supabase.channel(`room_timer_${roomId}`)
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'rooms', filter: `id=eq.${roomId}` }, (payload) => {
      if (payload.new.timer_status) timerState.value = payload.new.timer_status
    })
    .subscribe()

  // Local timer UI update loop
